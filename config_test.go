...
func localServer(t *testing.T) *Server {
	t.Helper()
	s, e := New(Config{Binary: "/bin/sh", SocketPath: filepath.Join(t.TempDir(), "s"), Env: []string{}, Dir: t.TempDir()})
	if e != nil {
		t.Fatal(e)
	}
	return s
...
func TestNewIsPureAndCopiesConfig(t *testing.T) {
	...
	s, e := New(Config{Binary: "fake", Dir: dir, Env: env, SocketName: "selected"})
	if e != nil {
		t.Fatal(e)
	}
	env[1] = "DATA=after"
...
func TestEnvironmentSelection(t *testing.T) {
	s, e := New(Config{Binary: "/bin/sh", Env: []string{"TMUX=/tmp/a,b,c,123,5", "TMUX_PANE=%9"}})
	if e != nil {
		t.Fatal(e)
	}
	...
	s, e = New(Config{Binary: "/bin/sh", SocketPath: "/tmp/explicit", Env: []string{"TMUX=malformed"}})
...
func TestEnvironmentNilVersusEmpty(t *testing.T) {
	...
	t.Setenv("TMUX", "")
	a, e := New(Config{Binary: "/bin/sh", SocketPath: "/tmp/a"})
	if e != nil {
		t.Fatal(e)
	}
	b, e := New(Config{Binary: "/bin/sh", SocketPath: "/tmp/a", Env: []string{}})
	if e != nil {
		t.Fatal(e)
	}
...
func TestParseEnvironment(t *testing.T) {
	...
	if _, e := ParseEnvironment(Environment{}); !errors.Is(e, ErrNotInsideTmux) {
		t.Fatal(e)
	}
	for _, s := range []string{"x", "/tmp/a,0,1", "relative,1,1", "/tmp/a,1,01", "/tmp/a,1,no"} {
		if _, e := ParseEnvironment(Environment{TMUX: s}); e == nil {
			t.Fatal("accepted", s)
		}
	}
}
...
func TestErrorRedactionAndEffects(t *testing.T) {
	...
	e := &CommandError{Command: "send-keys", Result: Result{Stderr: []byte(secret)}, Outcome: Outcome{Effect: Unknown}, Err: ErrOutputLimit}
