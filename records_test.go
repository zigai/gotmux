...
func TestGuardQueueAndUnwrap(t *testing.T) {
	s := localServer(t)
	g := &guard{identity: fixtureIdentity(s), links: []linkCheck{{session: "$1", index: 0, window: "@2"}}}
	p := g.wrap(emptyPlan(command("kill-pane", "-t", "%7")))
	argv, e := p.argv()
	if e != nil {
		t.Fatal(e)
	}
	if argv[0] != "if-shell" || !strings.Contains(strings.Join(argv, " "), "start_time") || !strings.Contains(strings.Join(argv, " "), "window_index") {
		t.Fatal(argv)
	}
	text, e := p.text()
	if e != nil {
	...
	_, e = g.unwrap(Result{Stdout: []byte(guardServerChanged)}, nil)
...
func TestNestedCommandEncoding(t *testing.T) {
	...
	g := &guard{identity: fixtureIdentity(s)}
...
func TestProbeFailureDoesNotDispatchRequestedMutation(t *testing.T) {
	e := opError("NewSession", &discoveryError{Err: &CommandError{Outcome: Outcome{Effect: Unknown}, Err: ErrNoServer}})
