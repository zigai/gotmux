...
func splitRecordFormat(fields []string) string {
	...
	for _, f := range fields {
		if f == "pane_current_command" {
			// pane_current_command reads the process name from /proc.
			// At the moment of split-window, the child process is forking/execing,
			// causing a race where #{n:...} and #{...} evaluate to different process names.
			// Emitting an empty string (0:,) avoids this race while keeping field count aligned.
			b.WriteString("0:,")
			continue
		}
