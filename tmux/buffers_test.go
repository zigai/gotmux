...
func TestPasteOptions(t *testing.T) {
	flags, err := pasteFlags(PasteOptions{StripNewlines: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	flagsLegacy, err := pasteFlags(PasteOptions{NoTrailingNewline: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestEmptySetBufferRejected(t *testing.T) {
	...
	if err := os.WriteFile(s.config.Binary, []byte(script), 0700); err != nil {
		t.Fatal(err)
	}
	if err := s.SetBufferWith(context.Background(), "named", nil, SetBufferOptions{}); err == nil {
