...
func executableFixture(t *testing.T, name string) string {
	...
	script := "#!/bin/sh\nprintf '%s\\000' \"$#\" \"$@\"\n"
	if e := os.WriteFile(path, []byte(script), 0o700); e != nil { //nolint:gosec // test script requires executable permissions
		t.Fatal(e)
	}
...
func TestExecArgvExact(t *testing.T) {
	for _, name := range []string{"program", "has space;$dollar", "-leading"} {
		...
		for _, args := range [][]string{nil, {""}, {"", "a b", "';$()", "#{pane_id}", "\xff"}} {
			assertExecArgvExact(t, path, args)
		}
	}
}

func assertExecArgvExact(t *testing.T, path string, args []string) {
	t.Helper()
	...
	out, e := exec.CommandContext(t.Context(), argv[0], argv[1:]...).Output()
	if e != nil {
		t.Fatal(e)
	}

	parts := bytes.Split(bytes.TrimSuffix(out, []byte{0}), []byte{0})
	if string(parts[0]) != strconv.Itoa(len(args)) {
		t.Fatalf("argc: %q", out)
	}
...
func TestUnsupportedDefaultProgramEnvironment(t *testing.T) {
	//nolint:exhaustruct_v5 // testing uninitialized default Program
